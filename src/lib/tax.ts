...
export function calculateTax(
  ...
  const regularMonthTaxRate = taxRates.find(
    (taxRate) => taxRate.minAmount <= monthlyGrossIncome,
  )!;
  ...
  const bonusMonthTaxRate = taxRates.find(
    (taxRate) => taxRate.minAmount <= bonusMonthIncome,
  )!;
  ...
  const decemberMonthTax =
    calculateYearlyTax(taxableIncome) - regularMonthTax * 10 - bonusMonthTax;
